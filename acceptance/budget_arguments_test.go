...
var _ = Describe("G2 budget argument compatibility matrix", func() {
	...
	DescribeTable("matches immutable grammar and precedence without native effects", func(args []string) {
		...
		Entry("leaf repeated known help", []string{"plan", "-hh"}), Entry("short attached value", []string{"plan", "-h=false"}), Entry("root attached help letter", []string{"-h=h"}), Entry("root attached repeated help", []string{"-h=hh"}), Entry("leaf attached help letter", []string{"plan", "-h=h"}), Entry("leaf attached repeated help", []string{"plan", "-h=hh"}), Entry("root empty help value", []string{"-h="}), Entry("leaf empty help value", []string{"plan", "-h="}), Entry("root attached help", []string{"--help=foo"}), Entry("long attached help", []string{"plan", "--help=false"}),
