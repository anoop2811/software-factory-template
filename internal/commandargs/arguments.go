// Package commandargs normalizes legacy argument grammar using Cobra metadata.
package commandargs
...
// SelectAction locates a registered command while preserving legacy help precedence.
func SelectAction(ctx context.Context, command *cobra.Command, args []string) (string, []string, bool, bool, error) {
	...
	for index, token := range tokens {
		...
		if token.positional || token.terminator {
			if !knownCommand(command, token.raw) {
				return "", nil, false, false, errors.New("expected a registered command")
			}
...
// Normalize classifies leaf options before Cobra parsing or any command effects.
func Normalize(ctx context.Context, command *cobra.Command, args []string) ([]string, bool, bool, error) {
	tokens, err := classify(ctx, command, args)
	if err != nil {
		return nil, false, false, err
	}
	...
	for index := 0; index < len(tokens); index++ {
		...
		if !allowedChoice(token.flag, value) {
...
// StringOption registers one scalar flag and its required metadata.
...
func knownCommand(root *cobra.Command, name string) bool {
...
func allowedChoice(flag *pflag.Flag, value string) bool {
	choices := flag.Annotations["factory.choices"]
	if len(choices) == 0 {
		return true
	}
...
// Choices attaches the domain's canonical alternatives to existing registered metadata.
func Choices(command *cobra.Command, name string, values []string) error {
	return command.Flags().SetAnnotation(name, "factory.choices", append([]string(nil), values...))
}
