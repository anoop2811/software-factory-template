// Package jsonvalue decodes bounded Python-compatible JSON without losing string or number identity.
package jsonvalue
...
func (DepthError) Error() string { return "JSON nesting exceeds private limit" }
...
scan:
	...
	if !strings.ContainsAny(string(number), ".eE") && len(bytes.TrimPrefix(raw, []byte{'-'})) > 4300 {
		return nil, SyntaxError{}
	}
	return number, nil
}
